function [t_grid, A_interp, R_interp] = main_SSA_ensemble(f,ka,kr,da,dr,eka,ekr,eda,edr,rho,X,MAX_ITER,period,N,NS,local,division)
...
for j=2:2:N
    if local==1
        nei=5;neighbor=[j-3 j-2 j-1 j+1 j+2];neighbor = mod(neighbor - 1, N) + 1;
        elseif local==0
...
for j=1:2:N
    if local==1
        nei=5;neighbor=[j-2 j-1 j+1 j+2 j+3];neighbor = mod(neighbor - 1, N) + 1;
    elseif local==0
        nei=N-1;neighbor=1:N;neighbor(j)=[];
    end 
    EA3(j)=sum(eq(X(neighbor),NSA));EA2(j)=sum(eq(X(neighbor),NSA-1));
...
%% Standardize trajectories for ensemble averaging
% Create a fixed time grid from 0 to the final time, step by 0.1
t_grid = 0:0.1:T(end-1);

% Interpolate the active (H3K4me3) and repressive (H3K27me3) counts
% A(:,1) and R(:,1) contain the counts for the fully methylated states (me3)
A_interp = interp1(T(1:iter-1), A(1:iter-1, 1), t_grid, 'previous');
R_interp = interp1(T(1:iter-1), R(1:iter-1, 1), t_grid, 'previous');
